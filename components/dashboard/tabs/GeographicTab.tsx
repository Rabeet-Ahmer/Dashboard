            ...
            <div className="h-[280px] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                <BarChart data={categoryData} margin={{ top: 10, right: 30, left: 0, bottom: 30 }}>
                  ...
                  <XAxis dataKey="category" tick={{ fontSize: 10 }} angle={-15} textAnchor="end" interval={0} />
          ...
          <CardContent className="pt-4 flex flex-col items-center justify-center">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
              {flagshipData.map((item, idx) => (
                <div key={item.flagship} className="flex items-center gap-1.5 text-xs text-muted-foreground px-2 py-0.5 rounded bg-muted/60 border border-border">
                  <span
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="border border-border bg-card shadow-2xs">
          ...
          <CardContent className="pt-4">
            <div className="h-[280px] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                <BarChart data={topBranches} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" opacity={0.15} horizontal={false} />
                  <XAxis type="number" tick={{ fontSize: 11 }} />
                  <YAxis type="category" dataKey="branchCode" tick={{ fontSize: 11 }} width={90} />
                  <Tooltip />
            ...
            <div className="h-[280px] w-full min-w-0">
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                <BarChart data={clusterData} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" opacity={0.15} horizontal={false} />
                  <XAxis type="number" tick={{ fontSize: 11 }} />
                  <YAxis type="category" dataKey="cluster" tick={{ fontSize: 10 }} width={140} />
                  <Tooltip />
